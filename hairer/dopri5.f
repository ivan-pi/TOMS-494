...
C
      MODULE DOPCOR_MOD
      USE DOPRI_MOD, ONLY: XOLD, HOUT,CDOPRI, FCN_PROC, SOLOUT_PROC
      USE HINIT_MOD
      IMPLICIT NONE
      PUBLIC :: DOPCOR
      CONTAINS
...
C --- INITIAL PREPARATIONS   
      ...
      write(IPRINT,*) "FIRST FUNCTION CALL"

      HMAX=ABS(HMAX)     
      IORD=5  
      IF (H.EQ.0.D0) H=HINIT(N,FCN,X,Y,XEND,POSNEG,K1,K2,K3,IORD,
     &                       HMAX,ATOL,RTOL,ITOL,RPAR,IPAR)
      write(IPRINT,*) "INITALIZED STEP"
      ...
      XOLD=X
      IF (IOUT.NE.0) THEN 
        write(IPRINT,*) "INSIDE IF"
          IRTRN=1
          HOUT=H

        print *, "NACCPT + 1 = ", NACCPT + 1
        print *, "XOLD = ", XOLD
        print *, "X = ", X
        print *, "Y = ", Y
        print *, "N = ", N
     ...
     &                RPAR,IPAR,IRTRN)
        STOP
        write(IPRINT,*) "CALLED SOLOUT"
        STOP
          IF (IRTRN.LT.0) GOTO 79
      ELSE
          IRTRN=0
      END IF
      write(IPRINT,*) "GOT PAST SOLOUT CALL"
...
C
      ...
      MODULE DOPRI5_ODE
        USE DOPCOR_MOD
        USE DOPRI_MOD, only: FCN_PROC, SOLOUT_PROC
        IMPLICIT NONE
...
C -------- CALL TO CORE INTEGRATOR ------------
      WRITE(IPRINT,*) "REACHED CORE INTEGRATOR"
