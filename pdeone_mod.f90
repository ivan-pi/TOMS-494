module pdeone_mod
    ...
    abstract interface
        ...
        subroutine bndry_interface(left,t,x,u,alpha,beta,gamm,npde)
            import wp
...
contains
    ...
    function new_pdeone(npde,x,icord,d,f,bndry) result(pde)
        ...
        print *, "Hello from PDEONE constructor"
...
!      common /mesh/ x(1)
!      common /coord/ icord
        integer :: icord, npts, npde
        real(wp), pointer :: x(:) => null()

        print *, "Hello from PDEONE"
    ...
    ! update the left boundary values
        call this%bndry(.true.,t, x(1), u, alpha, beta, gamma_, npde)
        itest = 0
        dxi = 1._wp/(x(2) - x(1))
        do k = 1, npde
  ...
  100   do k = 1, npde
    ...
    ! update the right boundary values
        call this%bndry(.false.,t,x(npts),u(1,npts),alpha,beta,gamma_,npde)
        itest = 0
        do k = 1, npde
            if (beta(k) /= 0.0_wp) cycle
